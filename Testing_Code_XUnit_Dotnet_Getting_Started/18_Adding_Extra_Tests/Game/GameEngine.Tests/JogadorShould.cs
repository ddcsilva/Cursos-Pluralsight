...
namespace GameEngine.Tests
{
    public class JogadorShould : IDisposable
    {
        ...
        [Fact]
        public void LevarDanoPequeno()
        {
            sut.LevarDano(1);

            Assert.Equal(99, sut.Saude);
        }

        [Fact]
        public void LevarDanoMedio()
        {
            sut.LevarDano(50);

            Assert.Equal(50, sut.Saude);
        }

        [Fact]
        public void TerNoMinimo1DeSaude()
        {
            {
                sut.LevarDano(101);

                Assert.Equal(1, sut.Saude);
            }
        }
    }
}
